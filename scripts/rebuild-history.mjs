...
const history = versions.map((version) => {
  ...
  return {
    version,
    publishedAt:
      release?.publishedAt ?? reconstructed[version]?.publishedAt ?? null,
    notes,
  };
});
...
for (const [version, { from, languages: targets }] of Object.entries(
  ...
  if (!source)
    throw new Error(`Missing localization source ${version}/${from}`);
...
for (const entry of history) {
  ...
  if (!simplified)
    throw new Error(`Missing Simplified Chinese ${entry.version}`);
  entry.notes["zh-TW"] = {
